...
The Linux Tauri build runs `node scripts/release/stage-pipewire-appimage.mjs` as its `beforeBuildCommand`. Hosted and local AppImage builds use `npm run tauri-appimage`, which invokes the normal Tauri build and then runs `scripts/release/patch-appimage-bootstrap.mjs` against the exact produced `AppDir`. On Linux, the staging hook uses `pkg-config` metadata from `libpipewire-0.3` and `libspa-0.2` to locate the library, prefix, PipeWire module, and SPA plugin directories. It fails before bundling unless those directories contain `spa-0.2/support/libspa-support.so`, `pipewire-0.3/libpipewire-module-protocol-native.so`, and `share/pipewire/client.conf`. It then stages every `libpipewire-0.3.so*` and `libjack.so*` file (including the required `libjack.so.0`), the complete SPA and PipeWire module trees, and the PipeWire config under `src-tauri/resources/pipewire/usr/lib` and `src-tauri/resources/pipewire/usr/share`. The hook is a no-op on non-Linux hosts.
...
After linuxdeploy generates its shell `AppRun` and native `AppRun.wrapped`, the patch step renames only the shell to `AppRun.shell`, installs a statically linked `scripts/release/appimage-bootstrap.c` as the new root `AppRun`, and repackages before updater signing. The bootstrap clears inherited `LD_LIBRARY_PATH` before `/usr/bin/env bash` can load it, sets `APPDIR` to the actual AppDir containing the bootstrap, preserves `APPIMAGE` and all application arguments, and leaves other environment variables unchanged. It requires a C compiler and static libc development archive; `CC` may select an architecture-matching compiler.
...
The gate requires the AppImage executable, static executable `AppRun`, executable `AppRun.shell`, preserved executable `AppRun.wrapped`, `usr/lib/libpipewire-0.3.so.0`, `usr/lib/libjack.so.0`, `usr/lib/spa-0.2/support/libspa-support.so`, `usr/lib/pipewire-0.3/libpipewire-module-protocol-native.so`, `usr/share/pipewire/client.conf`, and `usr/lib/gstreamer-1.0/libgstfluidsynthmidi.so`. It rejects an ELF interpreter in the bootstrap, runs it with a deliberately poisoned inherited `LD_LIBRARY_PATH` while checking APPIMAGE, APPDIR, and arguments, then compiles a temporary probe and runs it with the AppDir paths. The probe creates a PipeWire main loop/context using `client.conf` and loads the bundled GStreamer plugin with `dlopen`. This checks the first-hop loader boundary and runtime linkage/initialization rather than only checking file names.
...
Packaged builds check `https://github.com/openhoo/hooviestar/releases/latest/download/latest.json` on startup with a 30-second metadata timeout. When a newer signed version exists, Hooviestar downloads it with a bounded 30-minute timeout, verifies the Tauri signature, and presents it as ready. Installation is not automatic: the user must explicitly confirm `Installieren und neu starten`. Development builds never check for updates.

On confirmation, Hooviestar blurs the focused source field, settles pending source edits and mixer changes, and saves the current project before installation. If saving fails, installation is aborted without stopping the running engine; retryable failures leave the prepared update available to retry, while corrupt update data is rejected rather than installed. Saved project data is preserved across the restart. Apply changes in open dialogs before confirming the restart.
